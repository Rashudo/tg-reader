...
const config = {
  ...
  replies: {
    ...
    threadLimit: numFromEnv(process.env.REPLY_THREAD_LIMIT, 3),
    memes: (process.env.REPLY_MEMES || 'on').trim().toLowerCase() !== 'off',
    memeChoices: numFromEnv(process.env.REPLY_MEME_CHOICES, 20),
    memeCooldownH: numFromEnv(process.env.REPLY_MEME_COOLDOWN_H, 72),
